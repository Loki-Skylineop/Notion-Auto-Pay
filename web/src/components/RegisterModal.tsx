...
const MS_PLACEHOLDER = `По одной учётной записи на строку, поля разделены четырьмя дефисами:
...
export function RegisterModal({ open, onClose, onJobFinished }: Props) {
  ...
  const handleSubmit = async () => {
    setSubmitError(null)
    if (!activeProvider) {
      setSubmitError('Не найден доступный канал регистрации')
      return
    }
    if (!input.trim()) {
      setSubmitError('Вставьте хотя бы одну строку с учётными данными')
      return
    }
    const trimmedProxy = proxy.trim()
    if (trimmedProxy && !/^(https?|socks5h?):\/\//i.test(trimmedProxy)) {
      setSubmitError('URL прокси должен начинаться с http://, https://, socks5:// или socks5h://')
      return
    }
    setSubmitting(true)
    try {
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
          <div className="flex items-center gap-2 text-text-primary">
            ...
            <span className="text-[14px] font-semibold tracking-tight">Массовая регистрация аккаунтов</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-border bg-bg-secondary">
          {!job ? (
            <>
              <span className="text-[11px] text-text-muted">Закрытие окна не отменяет отправленную задачу. Её можно посмотреть в «Истории задач».</span>
    ...
    <div className="p-5 space-y-4">
      <div>
        <label className="block text-[12px] text-text-secondary mb-1.5 font-medium">
          {provider ? `Учётные данные ${provider.display}` : 'Учётные данные'}
        </label>
        <textarea
          value={input}
        ...
        <div className="flex justify-between items-center mt-1">
          <span className="text-[11px] text-text-muted">{lineCount} строк в очереди</span>
        </div>
      </div>

      <div className="flex items-center gap-3">
        <label className="text-[12px] text-text-secondary font-medium">Параллельность</label>
        <input
          type="number"
          min={1}
          value={concurrency}
        ...
        <span className="inline-flex items-center gap-1 text-[11px] text-text-muted">
          ...
          Рекомендуется ≤ 5 (антифрод MS/Notion)
        </span>
      </div>

      <div>
        <label className="block text-[12px] text-text-secondary mb-1.5 font-medium">
          URL прокси <span className="text-text-muted font-normal">(необязательно)</span>
        </label>
        <input
          type="text"
          value={proxy}
          ...
          placeholder={
            globalProxy
              ? `Пусто = глобальный прокси (${globalProxy})`
              : 'Пусто = напрямую; напр.: socks5://user:pass@host:port'
          }
        ...
        <div className="text-[11px] text-text-muted mt-1">
          Поддержка: socks5 / socks5h / http / https. Прокси здесь имеет приоритет; пусто —
          {globalProxy ? 'используется глобальный прокси (включён)' : 'напрямую (глобальный прокси не настроен)'}.
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            <span className="text-[13px] font-semibold tracking-tight">
              {job.state === 'running' ? 'Регистрация выполняется' : job.state === 'cancelled' ? 'Отменено' : 'Регистрация завершена'}
            </span>
            {job.state === 'running' && <IconSpinner size={13} className="animate-spin text-text-muted" />}
          </div>
          <span className="text-[12px] text-text-muted tabular-nums">
            {done} / {total} · OK {ok} · Ошибок {fail} · Параллельность {job.concurrency}
          </span>
        </div>
        ...
        {!streamConnected && job.state === 'running' && (
          <div className="text-[11px] text-warn flex items-center gap-1">
            <IconAlert size={11} /> Соединение потеряно, опрос… {streamError && `(${streamError})`}
          </div>
        )}
      </div>
...
function StepRow({ step, index }: { step: RegisterStep; index: number }) {
  ...
  switch (step.status) {
    case 'pending':
      statusEl = <span className="text-text-muted text-[11px]">Ожидание</span>
      break
    case 'running':
      statusEl = (
        <span className="inline-flex items-center gap-1 text-[11px] text-notion-blue">
          <IconSpinner size={11} className="animate-spin" /> Выполняется
        </span>
      )
      break
    case 'ok':
