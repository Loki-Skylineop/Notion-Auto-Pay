...
REM ============================================================
REM  Notion Auto Pay - сборка + запуск с доступом по локальной сети
REM
REM  Использование:
REM    buildloki.bat                       - собрать фронт + запустить сервер (LAN-доступ)
REM    buildloki.bat exe                   - только собрать notion-auto-pay.exe без запуска
REM    buildloki.bat run                   - собрать фронт и запустить
REM    buildloki.bat clean                 - удалить артефакты сборки
REM
REM  Защита панели паролем, rate limit включается автоматически:
REM    buildloki.bat --password=МойПароль       - запустить с паролем на вход в панель
REM    buildloki.bat run --password=МойПароль    - то же самое
REM    buildloki.bat --password МойПароль        - пробел вместо = тоже поддерживается
REM    set DASHBOARD_PASSWORD=МойПароль           - либо задать пароль через переменную окружения
REM
REM  Если пароль не задан, панель остаётся открытой: buildloki.bat передаёт серверу
...
echo ============================================
echo  Notion Auto Pay: сборка и запуск (LAN)
echo ============================================
...
if defined DASH_PASSWORD (
  ...
  echo Веб-панель открыта без пароля [--no-password]. Чтобы включить: buildloki.bat --password=ВашПароль
)
...
if /i "!LAN_NOTE!"=="admin" (
  ...
  echo buildloki.bat от имени администратора, чтобы открыть панель для телефона.
)
