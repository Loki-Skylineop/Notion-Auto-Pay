...
func TestAIRIConvertChatCompletionRequest_WithFilesToolsAndJSONSchema(t *testing.T) {
...
func TestAIRIAnthropicBridgeForwardsCorrelationHeaders(t *testing.T) {
	source := httptest.NewRequest("POST", "/v1/airi/chat/completions", nil)
	source.Header.Set("x-airi-session-id", "session-123")
	source.Header.Set("x-airi-round-id", "round-456")
	source.Header.Set("x-airi-app-surface", "stage-tamagotchi")

	bridged, err := newAnthropicBridgeRequest(source, &AnthropicRequest{
		Model:     AIRIModelAlias,
		MaxTokens: 32,
	...
	for key, want := range map[string]string{
		"x-airi-session-id":  "session-123",
		"x-airi-round-id":    "round-456",
		"x-airi-app-surface": "stage-tamagotchi",
	} {
		if got := bridged.Header.Get(key); got != want {
